...
bool CanHunt(){
    for(int i=0;i<n;i++) for(int j=0;j<n;j++){
        if(a[i][j] < BSLevel) return true;
    }
    return false;
}

int CanGo(Pos pos){
    queue<pair<Pos, int>> q; //position, dist
    bool visit[21][21];
...
int main(){
    ...
    {
        cin>>a[i][j];
        if(a[i][j] == 9) BSPos = {i, j};
    }

    while(CanHunt()) //사냥할 수 있는 먹이가 있을 때 400
    {
        //먹이가 1개라면 이거 선택 
        ...
        for(int i=0;i<n;i++) for(int j=0;j<n;j++){
            if(a[i][j] == 0 || a[i][j] == 9 || a[i][j] >= BSLevel) continue;
            int dist = CanGo({i, j});
            if(dist == 0) continue;
            pq.push({{i, j}, dist});
        }
